...
export default function PhaseDashboard() {
  ...
  // --- Loading / not found ---
  if (!currentPhase) {
    // Distinguish between "still loading" and "truly not found"
    const isLoading = loading || !currentProject
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3">
        {isLoading ? (
          ...
          <>
            ...
            <p className="text-gray-400 text-sm">Phase not found.</p>
            <button onClick={() => navigate(`/project/${projectId}`)} className="text-xs text-bx-accent hover:underline">
              Back to project
            </button>
          </>
        )}
